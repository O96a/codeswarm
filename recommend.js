...
async function recommendAgent(taskDescription, options = {}) {
  ...
  if (!await fs.pathExists(mehaisiDir)) {
    console.log(chalk.red('\n✗ Mehaisi not initialized. Run: mehaisi init\n'));
    return;
  }
  ...
  try {
    ...
    if (routing.agent) {
      ...
      console.log(chalk.white(`   mehaisi run ${routing.agent.name.toLowerCase().replace(/\s+/g, '-')}\n`));
    } else {
      console.log(chalk.yellow.bold('⚠ No suitable agent found\n'));
      console.log(chalk.white(`  Confidence: ${(routing.confidence * 100).toFixed(0)}%`));
      console.log(chalk.white(`  Reason: ${routing.reason}`));
